...
- `[13]` `restricted_admissions_by_approver` returns an empty breakdown, not a count of currently-restricted visits — was derived from the `restricted` field, now structurally zero. Confirmed in chat: the count is of admissions this build PERFORMS on restricted terms, and the only endpoint that performs one is `fallback-decision`, deferred with Phase 12. Seeded visitor C carries the restricted state so the dashboard flag renders, but she is a fixture rather than a record of an act. The scope section's "permanent zero" line is therefore correct as written and stays.
